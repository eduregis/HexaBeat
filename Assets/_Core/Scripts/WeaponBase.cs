...
public class WeaponBase : MonoBehaviour
{
    ...
    {
        Debug.Log($"{data.weaponName} (Nv.{currentLevel + 1}) fire!");
        // Aqui entra a lógica de instanciar projétil, golpe, etc.
    }

    public void Initialize(WeaponData weaponData, int level = 0)
    {
        data = weaponData;
    ...
    public void LevelUp()
    {
        ...
        {
            currentLevel++;
            Debug.Log($"{data.weaponName} subiu para Nv.{currentLevel + 1}");
        }
        else
        {
            Debug.Log($"{data.weaponName} já está no nível máximo!");
        }
    }
}